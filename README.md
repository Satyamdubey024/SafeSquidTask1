# 🖥️ Linux System Resource Monitoring Dashboard
...
- Uses `if [ "$usage_num" -ge 80 ]` to check usage and color the output red if over 80%.
...
```bash
./monitoring_system_resource.sh -disk -memory
```
